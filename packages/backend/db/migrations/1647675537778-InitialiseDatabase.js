...
module.exports = class InitialiseDatabase1647675537778 {
  name = 'InitialiseDatabase1647675537778'
  ...
  async up(queryRunner) {
    ...
    await queryRunner.query(
      `CREATE TABLE \`deal\` (\`id\` int NOT NULL AUTO_INCREMENT, \`originalPrice\` float(2) NOT NULL, \`discountPrice\` float(2) NOT NULL, \`description\` text NOT NULL, \`dealStartDate\` datetime NOT NULL, \`dealEndDate\` datetime NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`merchantOutletId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`deal\` ADD CONSTRAINT \`FK_9f8b6c5b9cee08c9376685e9a48\` FOREIGN KEY (\`merchantOutletId\`) REFERENCES \`merchant_outlet\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
    )
  }

  async down(queryRunner) {
    await queryRunner.query(
      `ALTER TABLE \`deal\` DROP FOREIGN KEY \`FK_9f8b6c5b9cee08c9376685e9a48\``
    )
    await queryRunner.query(
